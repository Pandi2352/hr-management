...
/** Quick-assign a reporting manager without opening the full edit form. */
export function AssignManagerModal({
  ...
  const [selectedId, setSelectedId] = useState(employee.managerId || '');
  const [isSaving, setIsSaving] = useState(false);

  useEffect(() => {
    if (!isOpen) return;
    setSelectedId(employee.managerId || '');
    setSearch('');
    setOptions([]);
  }, [isOpen, employee.managerId]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setOptions(((res.data || []) as ManagerOption[]).filter((m) => m._id !== employee._id));
      } catch {
        setOptions([]);
      } finally {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await employeesApi.updateEmployee(employee._id, { managerId: selectedId || undefined } as any);
      toast.success(
        selectedId ? 'Reporting manager updated.' : 'Reporting manager cleared (top level).',
      );
      onSaved();
      onClose();
    } catch (err: any) {
      toast.error(err?.response?.data?.message || 'Could not assign manager.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Assign Manager — ${displayName}`} className="max-w-md">
      <div className="space-y-3 pt-1">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search name, code or email (min 2 chars)…"
              ...
              return (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="font-mono text-[11px] text-slate-400">{m.employeeCode}</span>
                  </span>
